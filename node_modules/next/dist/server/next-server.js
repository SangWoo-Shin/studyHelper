...
const dynamicImportEsmDefault = processREMOVED_SECRETS.NEXT_MINIMAL ? (id)=>import(/* webpackIgnore: true */ id).then((mod)=>mod.default || mod) : (id)=>import(id).then((mod)=>mod.default || mod);
// For module that will be compiled to CJS, e.g. instrument
const dynamicRequire = processREMOVED_SECRETS.NEXT_MINIMAL ? __non_webpack_require__ : require;
...
class NextNodeServer extends _baseserver.default {
    constructor(options){
        ...
        this.handleNextImageRequest = async (req, res, parsedUrl)=>{
            ...
            if (this.minimalMode || this.nextConfig.output === "export" || processREMOVED_SECRETS.NEXT_MINIMAL) {
                res.statusCode = 400;
     ...
     * Using this from processREMOVED_SECRETS allows targeting SSR by calling
     * `processREMOVED_SECRETS.__NEXT_OPTIMIZE_CSS`.
     */ if (this.renderOpts.optimizeFonts) {
            processREMOVED_SECRETS.__NEXT_OPTIMIZE_FONTS = JSON.stringify(this.renderOpts.optimizeFonts);
        }
        if (this.renderOpts.optimizeCss) {
            processREMOVED_SECRETS.__NEXT_OPTIMIZE_CSS = JSON.stringify(true);
        }
        if (this.renderOpts.nextScriptWorkers) {
            processREMOVED_SECRETS.__NEXT_SCRIPT_WORKERS = JSON.stringify(true);
        }
        processREMOVED_SECRETS.NEXT_DEPLOYMENT_ID = this.nextConfig.deploymentId || "";
        if (!this.minimalMode) {
        ...
        if (this.serverOptions.experimentalTestProxy) {
            processREMOVED_SECRETS.NEXT_PRIVATE_TEST_PROXY = "true";
    ...
    async renderHTMLImpl(req, res, pathname, query, renderOpts) {
        if (processREMOVED_SECRETS.NEXT_MINIMAL) {
   ...
   */ async runMiddleware(params) {
    ...
    getPrerenderManifest() {
        ...
        if (((_this_renderOpts = this.renderOpts) == null ? void 0 : _this_renderOpts.dev) || ((_this_serverOptions = this.serverOptions) == null ? void 0 : _this_serverOptions.dev) || processREMOVED_SECRETS.NODE_ENV === "development" || processREMOVED_SECRETS.NEXT_PHASE === _constants.PHASE_PRODUCTION_BUILD) {
