...
const DEBUG = Boolean(processREMOVED_SECRETS.NEXT_PRIVATE_DEBUG_CACHE);
...
class FetchCache {
    ...
    static isAvailable(ctx) {
        return !!(ctx._requestHeaders["x-vercel-sc-host"] || processREMOVED_SECRETS.SUSPENSE_CACHE_URL);
    }
    constructor(ctx){
        this.headers = {};
        ...
        const scHost = ctx._requestHeaders["x-vercel-sc-host"] || processREMOVED_SECRETS.SUSPENSE_CACHE_URL;
        const scBasePath = ctx._requestHeaders["x-vercel-sc-basepath"] || processREMOVED_SECRETS.SUSPENSE_CACHE_BASEPATH;
        if (processREMOVED_SECRETS.SUSPENSE_CACHE_AUTH_TOKEN) {
            this.headers["Authorization"] = `Bearer ${processREMOVED_SECRETS.SUSPENSE_CACHE_AUTH_TOKEN}`;
        }
        if (scHost) {
            const scProto = processREMOVED_SECRETS.SUSPENSE_CACHE_PROTO || "https";
