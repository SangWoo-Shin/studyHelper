...
async function startServer(serverOptions) {
    ...
    await new Promise((resolve)=>{
        server.on("listening", async ()=>{
            ...
            processREMOVED_SECRETS.PORT = port + "";
            processREMOVED_SECRETS.__NEXT_PRIVATE_ORIGIN = appUrl;
            // Only load env and config in dev to for logging purposes
            let envInfo;
            let expFeatureInfo;
            if (isDev) {
                const startServerInfo = await (0, _appinfolog.getStartServerInfo)(dir, isDev);
                envInfo = startServerInfoREMOVED_SECRETSInfo;
            ...
            try {
                ...
                if (!processREMOVED_SECRETS.NEXT_MANUAL_SIG_HANDLE) {
                ...
                if (processREMOVED_SECRETS.TURBOPACK) {
    ...
    if (isDev) {
        ...
        watchConfigFiles(dir, async (filename)=>{
            if (processREMOVED_SECRETS.__NEXT_DISABLE_MEMORY_WATCHER) {
...
if (processREMOVED_SECRETS.NEXT_PRIVATE_WORKER && process.send) {
