...
function onReport(metric) {
    ...
    if (processREMOVED_SECRETS.NODE_ENV === "production" && // This field is empty unless you explicitly configure it:
    processREMOVED_SECRETS.__NEXT_ANALYTICS_ID) {
        var _window___NEXT_DATA__;
        const body = {
            dsn: processREMOVED_SECRETS.__NEXT_ANALYTICS_ID,
            id: metric.id,
...
export default ((onPerfEntry)=>{
    if (processREMOVED_SECRETS.__NEXT_ANALYTICS_ID) {
        ...
        isRegistered = true;
        const attributions = processREMOVED_SECRETS.__NEXT_WEB_VITALS_ATTRIBUTION;
        for (const webVital of WEB_VITALS){
            try {
                let mod;
                if (processREMOVED_SECRETS.__NEXT_HAS_WEB_VITALS_ATTRIBUTION) {
