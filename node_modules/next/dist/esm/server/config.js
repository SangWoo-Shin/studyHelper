...
function assignDefaults(dir, userConfig, silent) {
    ...
    if (((_result_experimental = result.experimental) == null ? void 0 : _result_experimental.ppr) && !"14.2.7".includes("canary") && !processREMOVED_SECRETS.__NEXT_TEST_MODE) {
    ...
    if (processREMOVED_SECRETS.NEXT_DEPLOYMENT_ID) {
        result.deploymentId = processREMOVED_SECRETS.NEXT_DEPLOYMENT_ID;
    }
...
export default async function loadConfig(phase, dir, { customConfig, rawConfig, silent = true, onLoadUserConfig } = {}) {
    if (!processREMOVED_SECRETS.__NEXT_PRIVATE_RENDER_WORKER) {
        try {
            loadWebpackHook();
        } catch (err) {
            // this can fail in standalone mode as the files
            // aren't traced/included
            if (!processREMOVED_SECRETS.__NEXT_PRIVATE_STANDALONE_CONFIG) {
                throw err;
            }
        }
    }
    if (processREMOVED_SECRETS.__NEXT_PRIVATE_STANDALONE_CONFIG) {
        return JSON.parse(processREMOVED_SECRETS.__NEXT_PRIVATE_STANDALONE_CONFIG);
    }
    ...
    if (processREMOVED_SECRETS.__NEXT_PRIVATE_RENDER_WORKER_CONFIG) {
        return JSON.parse(processREMOVED_SECRETS.__NEXT_PRIVATE_RENDER_WORKER_CONFIG);
    }
    const curLog = silent ? {
        warn: ()=>{},
        info: ()=>{},
    ...
    if (path == null ? void 0 : path.length) {
        ...
        try {
            const envBefore = Object.assign({}, processREMOVED_SECRETS);
            // `import()` expects url-encoded strings, so the path must be properly
            // escaped and (especially on Windows) absolute paths must pe prefixed
            // with the `file://` protocol
            if (processREMOVED_SECRETS.__NEXT_TEST_MODE === "jest") {
            ...
            const newEnv = {};
            for (const key of Object.keys(processREMOVED_SECRETS)){
                if (envBefore[key] !== processREMOVED_SECRETS[key]) {
                    newEnv[key] = processREMOVED_SECRETS[key];
                }
            }
