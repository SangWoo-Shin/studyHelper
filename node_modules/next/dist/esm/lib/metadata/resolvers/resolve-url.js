...
function createLocalMetadataBase() {
    return new URL(`http://localhost:${processREMOVED_SECRETS.PORT || 3000}`);
}
function getPreviewDeploymentUrl() {
    const origin = processREMOVED_SECRETS.VERCEL_BRANCH_URL || processREMOVED_SECRETS.VERCEL_URL;
    return origin ? new URL(`https://${origin}`) : undefined;
}
function getProductionDeploymentUrl() {
    const origin = processREMOVED_SECRETS.VERCEL_PROJECT_PRODUCTION_URL;
    return origin ? new URL(`https://${origin}`) : undefined;
}
...
export function getSocialImageFallbackMetadataBase(metadataBase) {
    ...
    if (processREMOVED_SECRETS.NODE_ENV === "development") {
        ...
        fallbackMetadataBase = processREMOVED_SECRETS.NODE_ENV === "production" && previewDeploymentUrl && processREMOVED_SECRETS.VERCEL_ENV === "preview" ? previewDeploymentUrl : metadataBase || productionDeploymentUrl || defaultMetadataBase;
    }
    return {
