...
export function getCacheDirectory(fileDirectory, envPath) {
    ...
    if (envPath) {
        ...
        if (process.platform === "linux") {
            systemCacheDirectory = processREMOVED_SECRETS.XDG_CACHE_HOME || path.join(os.homedir(), ".cache");
        } else if (process.platform === "darwin") {
            systemCacheDirectory = path.join(os.homedir(), "Library", "Caches");
        } else if (process.platform === "win32") {
            systemCacheDirectory = processREMOVED_SECRETS.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local");
        } else {
    ...
    if (!path.isAbsolute(result)) {
        ...
        result = path.resolve(processREMOVED_SECRETS["INIT_CWD"] || process.cwd(), result);
    }
    return result;
}
