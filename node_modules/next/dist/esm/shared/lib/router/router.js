...
function getMiddlewareData(source, response, options) {
    const nextConfig = {
        ...
        trailingSlash: Boolean(processREMOVED_SECRETS.__NEXT_TRAILING_SLASH)
    };
    ...
    if (rewriteTarget) {
        if (rewriteTarget.startsWith("/") || processREMOVED_SECRETS.__NEXT_EXTERNAL_MIDDLEWARE_REWRITE_RESOLVE) {
            ...
            return Promise.all([
                ...
                if (isDynamicRoute(as) || !rewriteHeader && pages.includes(normalizeLocalePath(removeBasePath(as), options.router.locales).pathname)) {
                    const parsedSource = getNextPathnameInfo(parseRelativeUrl(source).pathname, {
                        nextConfig: processREMOVED_SECRETS.__NEXT_HAS_REWRITES ? undefined : nextConfig,
                        parseData: true
                    });
                    as = addBasePath(parsedSource.pathname);
                    parsedRewriteTarget.pathname = as;
                }
                if (processREMOVED_SECRETS.__NEXT_HAS_REWRITES) {
...
const manualScrollRestoration = processREMOVED_SECRETS.__NEXT_SCROLL_RESTORATION && typeof window !== "undefined" && "scrollRestoration" in window.history && !!function() {
    try {
        let v = "__next";
...
function fetchNextData(param) {
    ...
    const getData = (params)=>{
        ...
        return fetchRetry(dataHref, isServerRender ? 3 : 1, {
            ...
            if (!persistCache || processREMOVED_SECRETS.NODE_ENV !== "production" || data.response.headers.get("x-middleware-cache") === "no-cache") {
...
class Router {
   ...
   */ push(url, as, options) {
        ...
        if (processREMOVED_SECRETS.__NEXT_SCROLL_RESTORATION) {
   ...
   */ replace(url, as, options) {
    ...
    async _bfl(as, resolvedAs, locale, skipNavigate) {
        if (processREMOVED_SECRETS.__NEXT_CLIENT_ROUTER_FILTER_ENABLED) {
    ...
    async change(method, url, as, options, forcedScroll) {
        ...
        if (processREMOVED_SECRETS.__NEXT_I18N_SUPPORT) {
        ...
        if (shouldResolveHref && pathname !== "/_error") {
            ...
            if (processREMOVED_SECRETS.__NEXT_HAS_REWRITES && as.startsWith("/")) {
        ...
        if (!isLocalURL(as)) {
            if (processREMOVED_SECRETS.NODE_ENV !== "production") {
     ...
     */ let route = requestedRoute;
        try {
            ...
            let cachedRouteInfo = existingInfo && !("initial" in existingInfo) && processREMOVED_SECRETS.NODE_ENV !== "development" ? existingInfo : undefined;
            ...
            if (!this.isPreview && routeInfo.__N_SSG && processREMOVED_SECRETS.NODE_ENV !== "development" && !isQueryUpdating) {
   ...
   */ async prefetch(url, asPath, options) {
        ...
        if (processREMOVED_SECRETS.__NEXT_HAS_REWRITES && asPath.startsWith("/")) {
            let rewrites;
        ...
        const data = processREMOVED_SECRETS.__NEXT_MIDDLEWARE_PREFETCH === "strict" ? null : await withMiddlewareEffects({
        ...
        await Promise.all([
            this.pageLoader._isSsg(route).then((isSsg)=>{
                return isSsg ? fetchNextData({
                    ...
                    persistCache: !this.isPreview,
                    isPrefetch: true,
                    unstable_skipClientCache: options.unstable_skipClientCache || options.priority && !!processREMOVED_SECRETS.__NEXT_OPTIMISTIC_CLIENT_CACHE
    ...
    constructor(pathname, query, as, { initialProps, pageLoader, App, wrapApp, Component, err, subscription, isFallback, locale, locales, defaultLocale, domainLocales, isPreview }){
        ...
        if (processREMOVED_SECRETS.__NEXT_CLIENT_ROUTER_FILTER_ENABLED) {
            ...
            const routerFilterSValue = processREMOVED_SECRETS.__NEXT_CLIENT_ROUTER_S_FILTER;
            const staticFilterData = routerFilterSValue ? routerFilterSValue : undefined;
            const routerFilterDValue = processREMOVED_SECRETS.__NEXT_CLIENT_ROUTER_D_FILTER;
        ...
        this.basePath = processREMOVED_SECRETS.__NEXT_ROUTER_BASEPATH || "";
        ...
        this.isReady = !!(self.__NEXT_DATA__.gssp || self.__NEXT_DATA__.gip || self.__NEXT_DATA__.isExperimentalCompile || self.__NEXT_DATA__.appGip && !self.__NEXT_DATA__.gsp || !autoExportDynamic && !self.location.search && !processREMOVED_SECRETS.__NEXT_HAS_REWRITES);
        if (processREMOVED_SECRETS.__NEXT_I18N_SUPPORT) {
        ...
        this.state = {
            ...
            isPreview: !!isPreview,
            locale: processREMOVED_SECRETS.__NEXT_I18N_SUPPORT ? locale : undefined,
            isFallback
        };
