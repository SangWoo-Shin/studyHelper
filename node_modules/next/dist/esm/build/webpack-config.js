...
const nodePathList = (processREMOVED_SECRETS.NODE_PATH || "").split(process.platform === "win32" ? ";" : ":").filter((p)=>!!p);
...
export default async function getBaseWebpackConfig(dir, { buildId, encryptionKey, config, compilerType, dev = false, entrypoints, isDevFallback = false, pagesDir, reactProductionProfiling = false, rewrites, originalRewrites, originalRedirects, runWebpackSpan, appDir, middlewareMatchers, noMangling = false, jsConfig, resolvedBaseUrl, supportedBrowsers, clientRouterFilters, fetchCacheKeyPrefix, edgePreviewProps }) {
    ...
    const terserOptions = {
        ...
        mangle: {
            ...
            ...processREMOVED_SECRETS.__NEXT_MANGLING_DEBUG || noMangling ? {
                toplevel: true,
                module: true,
    ...
    let webpackConfig = {
        parallelism: Number(processREMOVED_SECRETS.NEXT_WEBPACK_PARALLELISM) || undefined,
        ...isNodeServer ? {
            externalsPresets: {
                node: true
            }
        ...
        plugins: [
            ...
            // replacement is done before its processREMOVED_SECRETS.* handling
    ...
    if (processREMOVED_SECRETS.NEXT_WEBPACK_LOGGING) {
        const infra = processREMOVED_SECRETS.NEXT_WEBPACK_LOGGING.includes("infrastructure");
        const profileClient = processREMOVED_SECRETS.NEXT_WEBPACK_LOGGING.includes("profile-client");
        const profileServer = processREMOVED_SECRETS.NEXT_WEBPACK_LOGGING.includes("profile-server");
        const summaryClient = processREMOVED_SECRETS.NEXT_WEBPACK_LOGGING.includes("summary-client");
        const summaryServer = processREMOVED_SECRETS.NEXT_WEBPACK_LOGGING.includes("summary-server");
