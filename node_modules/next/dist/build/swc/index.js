...
const infoLog = (...args)=>{
    if (processREMOVED_SECRETS.NEXT_PRIVATE_BUILD_WORKER) {
        return;
    }
    if (processREMOVED_SECRETS.DEBUG) {
        _log.info(...args);
    }
};
...
const __INTERNAL_CUSTOM_TURBOPACK_BINDINGS = processREMOVED_SECRETS.__INTERNAL_CUSTOM_TURBOPACK_BINDINGS;
...
async function loadBindings(useWasmBinary = false) {
    ...
    if (!processREMOVED_SECRETS.RUST_MIN_STACK) {
        processREMOVED_SECRETS.RUST_MIN_STACK = "8388608";
    }
    if (pendingBindings) {
    ...
    pendingBindings = new Promise(async (resolve, _reject)=>{
        ...
        let attempts = [];
        const disableWasmFallback = processREMOVED_SECRETS.NEXT_DISABLE_SWC_WASM;
   ...
   */ function subscribe(useBuffer, nativeFunction) {
    ...
    async function rustifyProjectOptions(options) {
        return {
            ...
            env: optionsREMOVED_SECRETS && rustifyEnv(optionsREMOVED_SECRETS),
