...
function turborepoTraceAccess(f, parent) {
    ...
    // function.
    if (!processREMOVED_SECRETS.TURBOREPO_TRACE_FILE) return f();
...
async function writeTurborepoAccessTraceResult({ distDir, traces }) {
    const configTraceFile = processREMOVED_SECRETS.TURBOREPO_TRACE_FILE;
...
async function withTurborepoTraceAccess(f) {
    ...
    const restoreEnv = (0, _envREMOVED_SECRETSProxy)(envVars);
    let functionResult;
