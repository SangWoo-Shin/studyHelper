...
async function getStaticAssetRouteCode(resourcePath, fileBaseName) {
    const cache = fileBaseName === "favicon" ? "public, max-age=0, must-revalidate" : processREMOVED_SECRETS.NODE_ENV !== "production" ? cacheHeader.none : cacheHeader.longCache;
    const code = `\
/* static asset route */
...
async function getDynamicSiteMapRouteCode(resourcePath, page, loaderContext) {
    ...
    if (processREMOVED_SECRETS.NODE_ENV === "production" && hasGenerateSiteMaps && page.includes("[__metadata_id__]")) {
...
export async function GET(_, ctx) {
  ...
  const targetId = processREMOVED_SECRETS.NODE_ENV !== 'production'
    ? __metadata_id__?.[0]
    : __metadata_id__
